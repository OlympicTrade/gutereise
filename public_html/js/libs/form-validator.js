    ...
    this.checkErrors = function(form, options) {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp['errors']['form']) {
                    console.log(formV.renderErrors(resp['errors']['form']));
                    $('.form-errors', form).append(formV.renderErrors(resp['errors']['form']));
                }

                form.find('input[type!="submit"][type!="hidden"], select, textarea').each(function () {
                    var input = $(this);
